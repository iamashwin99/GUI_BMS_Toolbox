...
% 2015.
function results = setupSimVehicleGUI(files, cellParameter,moduleParameter, packParameter, motorParameter, wheelParameter, drivetrainParameter, vehicleParameter)
 % files = {'nycc.txt','udds.txt','us06.txt','hwfet.txt','kt.txt'}; % drive cycles

  % Setup the Chevy Volt-like vehicle
  % set up cell: capacity [Ah], weight [g], (vmax, vnom, vmin) [V]
  cell = setupCell(cellParameter.capacity,cellParameter.weight,cellParameter.vmax,cellParameter.vnom,cellParameter.vmin);
  % set up module: numParallel, numSeries, overhead for this cell by weight
  module = setupModule(moduleParameter.numParallel,moduleParameter.numSeries,moduleParameter.overHead,cell);
  % set up pack: numSeries, overhead by weight, (full SOC, empty SOC) [%],
  % efficiency for this module
  pack = setupPack(packParameter.numSeries,packParameter.overHead,packParameter.fullsoc,packParameter.emptysoc,packParameter.efficiency,module);
  % set up motor: Lmax [Nm], (RPMrated, RPMmax) [RPM], efficiency,
  % inertia [kg/m2]
  motor = setupMotor(motorParameter.lmax,motorParameter.ratedRPM,motorParameter.maxRPM,motorParameter.efficiency,motorParameter.inertia);
  % set up wheel: radius [m], inertia [kg/m2], rollCoef
  wheel = setupWheel(wheelParameter.radius,wheelParameter.inertia,wheelParameter.rollCoeff);
  ...
  % motor, and wheel 
  drivetrain = setupDrivetrain(drivetrainParameter.invertereff,drivetrainParameter.fracregen,drivetrainParameter.gearratio,drivetrainParameter.gearinertia,drivetrainParameter.gearefficiency,pack,motor,wheel);
  % set up vehicle: # wheels, roadForce [N], Cd, frontal area [m2], weight
  % [kg], payload [kg], overhead power [W] for this drivetrain
  vehicle = setupVehicle(vehicleParameter.wheels,vehicleParameter.roadForce,vehicleParameter.cd,vehicleParameter.area,vehicleParameter.weight,vehicleParameter.payload,vehicleParameter.overheadpower,drivetrain);
